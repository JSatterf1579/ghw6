...
Vertex* ClipPolygon(int count, Vertex* input, int* out_count, int hpositive)
{
	//Vertex* output = new Vertex[2 * count];
	Vertex* output;
	//Clip against h>0
	if(hpositive == 1){
		output = ClipOnH(1, count, input, out_count);
	} else {
		output = ClipOnH(-1, count, input, out_count);
	}
	//Clip x positive
	// output = ClipOnEdge(1.0f,0.0f,0.0f,*out_count,output,out_count);
	//Clip x negative
	// output = ClipOnEdge(-1.0f,0.0f,0.0f,*out_count,output,out_count);
	//Clip y positive
	output = ClipOnEdge(0.0f,1.0f,0.0f,*out_count,output,out_count);
	//Clip y negative
	output = ClipOnEdge(0.0f,-1.0f,0.0f,*out_count,output,out_count);
	//Clip z positive
	output = ClipOnEdge(0.0f,0.0f,1.0f,*out_count,output,out_count);
	//Clip z negative
	output = ClipOnEdge(0.0f,0.0f,-1.0f,*out_count,output,out_count);
...
Vertex* FindIntersection(Vertex* v1, Vertex* v2, Vertex* edge)
{
	Vertex *clippingPoint = new Vertex();


	float tIntersect;
	if (edge->x != 0)
	{
		tIntersect = (v1->h + v1->x) / ((v1->h + v1->x) - (v2->h + v2->x));
	}
	else if (edge->y != 0)
	{
		tIntersect = (v1->h + v1->y) / ((v1->h + v1->y) - (v2->h + v2->y));
	}
	else if (edge->z != 0)
	{
		tIntersect = (v1->h + v1->z) / ((v1->h + v1->z) - (v2->h + v2->z));
	}
	else
	{
		tIntersect = v1->h / (v1->h - v2->h);
	}
	clippingPoint->x = (1-tIntersect) * v1->x + v2->x * tIntersect;
	clippingPoint->y = (1 - tIntersect) * v1->y + v2->y * tIntersect;
	clippingPoint->z = (1 - tIntersect) * v1->z + v2->z * tIntersect;
	clippingPoint->h = (1 - tIntersect) * v1->h + v2->h * tIntersect;
	/*if (edge->h == 1)
	{
		clippingPoint->h = 1;
	}
	...
	{*/
		// clippingPoint->h = v1->h + hslope*tIntersect;
	//}
	return clippingPoint;
}

Vertex* ClipOnEdge(float edgex, float edgey, float edgez, int count, Vertex* input, int* out_count)
{
	int currentOutput = 0;
	float v1Component = 0;
	float v2Component = 0;
	float edgeComponent = 0;
	Vertex* output = new Vertex[2 * count];
	for (int i = 0; i < count; i++)
	{
		...
		v1Component = abs(v1Component/p1.h);
		v2Component = abs(v2Component/p2.h);
		edgeComponent = abs(edgeComponent);


		if (v1Component > edgeComponent)
		{
			if (v2Component < edgeComponent)
			{
				Vertex *edge = new Vertex();
				edge->x = edgex;
				edge->y = edgey;
				edge->z = edgez;
				output[currentOutput] = *FindIntersection(&p1, &p2, edge);
				currentOutput++;
				output[currentOutput] = p2;
				currentOutput++;
			}
			else
			{
				//No points added if both outside
			}
		}
		else
		{
	...
	*out_count = currentOutput;
	return output;
}

Vertex* ClipOnH(float sign, int count, Vertex* input, int* out_count)
{
	int currentOutput = 0;
	float edge = 0.00001;
	Vertex* output = new Vertex[2 * count];
	for (int i = 0; i < count; i++)
	{
		...
		else
		{
			...
			else
			{
				...
				edge1->h = edge;
				output[currentOutput] = *FindIntersection(&p1, &p2, edge1);
				currentOutput++;
			}
		}
	}
